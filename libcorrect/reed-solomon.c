...
correct_reed_solomon *correct_reed_solomon_create(field_operation_t primitive_polynomial, field_logarithm_t first_consecutive_root, field_logarithm_t generator_root_gap, size_t num_roots) {
    correct_reed_solomon *rs = jo_malloc(sizeof(correct_reed_solomon));
    if(rs == NULL)
    {
        return NULL;
    }

    jo_memset(rs, 0, sizeof(correct_reed_solomon));
    ...
    rs->generator_roots = jo_malloc(rs->min_distance * sizeof(field_element_t));

    if(rs->generator_roots == NULL)
    {
        return NULL;
    }
...
void correct_reed_solomon_destroy(correct_reed_solomon *rs) {
    ...
    if (rs->has_init_decode) {
        ...
        jo_free(rs->error_roots);
        jo_free(rs->error_vals);
        jo_free(rs->error_locations);
        ...
        for (unsigned int i = 0; i < rs->min_distance; i++) {
            jo_free(rs->generator_root_exp[i]);
        }
        jo_free(rs->generator_root_exp);
        for (field_operation_t i = 0; i < 256; i++) {
            jo_free(rs->element_exp[i]);
        }
        jo_free(rs->element_exp);
